...
from ufl.corealg.map_dag import map_expr_dag, map_expr_dag_legacy
from ufl.form import BaseForm, Form, FormSum, ZeroBaseForm
from ufl.integral import Integral


def map_integrands_legacy(function, form, only_integral_type=None):
    """Map integrands.
    ...
    if isinstance(form, Form):
        mapped_integrals = [
            map_integrands_legacy(function, itg, only_integral_type) for itg in form.integrals()
        ]
        nonzero_integrals = [
        ...
        mapped_components = [
            map_integrands_legacy(function, component, only_integral_type)
        ...
        return Adjoint(map_integrands_legacy(function, form._form, only_integral_type))
    elif isinstance(form, Action):
        left = map_integrands_legacy(function, form._left, only_integral_type)
        right = map_integrands_legacy(function, form._right, only_integral_type)
        # Zeros are caught inside `Action.__new__`
        return Action(left, right)
    elif isinstance(form, ZeroBaseForm):
        arguments = tuple(
            map_integrands_legacy(function, arg, only_integral_type) for arg in form._arguments
        )
...
def map_integrand_dags_legacy(function, form, only_integral_type=None, compress=True):
    """Map integrand dags."""
    return map_integrands_legacy(
        lambda expr: map_expr_dag_legacy(function, expr, compress), form, only_integral_type
    )
