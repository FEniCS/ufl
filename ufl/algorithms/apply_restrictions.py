...
def make_domain_integral_type_map(integral_data):
    ...
    if integration_domain in domain_integral_type_dict:
        if domain_integral_type_dict[integration_domain] != integration_type:
            raise RuntimeError(f"Found inconsistent integral types for the integration domain ({integration_domain}): {domain_integral_type_dict[integration_domain]} != {integration_type}")
    else:
...
class ToBeRestrectedReplacer(MultiFunction):
    ...
    def to_be_restricted(self, o):
        ...
        if isinstance(domain, MixedMesh):
            raise RuntimeError(f"Not expecting a (modified) terminal object on a mixed mesh at this stage: got {repr(o)}")
