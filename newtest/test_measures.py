...
def test_foo():
    ...
    #assert dSd.domain().data() == { "mesh": mesh, "interior_facet": interior_facet_domains }
    ...
    domain, = Ms.domains()
    assert domain.data() == mymesh
    assert Ms.subdomain_data()[mydomain]["exterior_facet"] == exterior_facet_domains

    domain, = MS.domains()
    assert domain.data() == mymesh
    assert MS.subdomain_data()[mydomain]["interior_facet"] == interior_facet_domains

    # Test joining of these domains in a single form
    domain, = M.domains()
    assert domain.data() == mymesh
    assert M.subdomain_data()[mydomain]["cell"] == cell_domains
    assert M.subdomain_data()[mydomain]["exterior_facet"] == exterior_facet_domains
    assert M.subdomain_data()[mydomain]["interior_facet"] == interior_facet_domains
