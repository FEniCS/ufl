...
class FormOperations(UflTestCase):
    ...
    def test_lhs_rhs_simple(self):
        ...
        F0 = f*u*v*w*dx
        a, L = system(F0)
        assert(len(a._integrals) == 0)
        assert(len(L._integrals) == 0)
    ...
    def test_lhs_rhs_derivatives(self):
        V = FiniteElement("CG", interval, 1)
        v = TestFunction(V)
        u = TrialFunction(V)
        f = Coefficient(V)
        ...
        a, L = system(F0)
        assert(len(a._integrals) == 1)
        assert(len(L._integrals) == 3)

        F1 = derivative(F0, f)
        a, L = system(F0)

    def test_lhs_rhs_slightly_obscure(self):
