...
def test_construct_domains_from_cells():
    for cell in all_cells:
        ...
        Mesh(FiniteElement("Lagrange", cell, 1, (d, ), identity_pull_back, H1))


def test_construct_domains_with_names():
    for cell in all_cells:
        d = cell.geometric_dimension()
        e = FiniteElement("Lagrange", cell, 1, (d, ), identity_pull_back, H1)
        D2 = Mesh(e, ufl_id=2)
        D3 = Mesh(e, ufl_id=3)
        D3b = Mesh(e, ufl_id=3)
        assert D2 != D3
...
def test_topdomain_creation():
    D = Mesh(FiniteElement("Lagrange", interval, 1, (1, ), identity_pull_back, H1))
    assert D.geometric_dimension() == 1
    D = Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1))
    assert D.geometric_dimension() == 2
    D = Mesh(FiniteElement("Lagrange", tetrahedron, 1, (3, ), identity_pull_back, H1))
...
def test_cell_legacy_case():
    ...
    V = FiniteElement("Lagrange", triangle, 1, (), identity_pull_back, H1)
...
def test_simple_domain_case():
    ...
    D = Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=3)

    V = FunctionSpace(D, FiniteElement("Lagrange", D.ufl_cell(), 1, (), identity_pull_back, "H1"))
    f = Coefficient(V)
    assert f.ufl_domains() == (D, )
...
def test_join_domains():
    ...
    xa = FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1)
    xb = FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1)

    # Equal domains are joined
    assert 1 == len(join_domains([
        Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=7),
        Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=7)]))
    assert 1 == len(join_domains([
        Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=7, cargo=mesh7),
        Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=7, cargo=mesh7)]))
    ...
    assert 2 == len(join_domains([
        Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=7),
        Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=8)]))
    assert 2 == len(join_domains([
        Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=7),
        Mesh(FiniteElement("Lagrange", quadrilateral, 1, (2, ), identity_pull_back, H1), ufl_id=8)]))
    ...
    xc = Coefficient(FunctionSpace(
        Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1)),
        FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1)))
    ...
    assert 2 == len(join_domains([
        Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=7, cargo=mesh7),
        Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=8, cargo=mesh8)]))
    assert 2 == len(join_domains([
        Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=7, cargo=mesh7),
        Mesh(FiniteElement("Lagrange", quadrilateral, 1, (2, ), identity_pull_back, H1),
             ufl_id=8, cargo=mesh8)]))
    # Geometric dimensions must match
    with pytest.raises(BaseException):
        join_domains([
            Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1)),
            Mesh(FiniteElement("Lagrange", triangle3, 1, (3, ), identity_pull_back, H1))])
    with pytest.raises(BaseException):
        join_domains([
            Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=7, cargo=mesh7),
            Mesh(FiniteElement("Lagrange", triangle3, 1, (3, ), identity_pull_back, H1), ufl_id=8, cargo=mesh8)])
    ...
    # Nones are removed
    assert 2 == len(join_domains([
        None, Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=3),
        None, Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=3),
        None, Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=4)]))
    assert 2 == len(join_domains([
        Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1), ufl_id=7), None,
        Mesh(FiniteElement("Lagrange", quadrilateral, 1, (2, ), identity_pull_back, H1), ufl_id=8)]))
    assert None not in join_domains([
        Mesh(FiniteElement("Lagrange", triangle3, 1, (3, ), identity_pull_back, H1), ufl_id=7), None,
        Mesh(FiniteElement("Lagrange", tetrahedron, 1, (3, ), identity_pull_back, H1), ufl_id=8)])


def test_everywhere_integrals_with_backwards_compatibility():
    D = Mesh(FiniteElement("Lagrange", triangle, 1, (2, ), identity_pull_back, H1))

    V = FunctionSpace(D, FiniteElement("Lagrange", triangle, 1, (), identity_pull_back, H1))
    f = Coefficient(V)
...
def test_merge_sort_integral_data():
    ...
    V = FunctionSpace(D, FiniteElement("CG", triangle, 1, (), identity_pull_back, H1))
