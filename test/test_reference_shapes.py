...
def test_reference_shapes():
    ...
    assert S.value_shape(domain) == (3, 3)
    assert S.reference_value_shape == (6,)

    M = MixedElement([V, U, W])
    assert M.value_shape(domain) == (7,)
