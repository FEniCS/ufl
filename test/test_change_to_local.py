...
class TestChangeToLocal(UflTestCase):
    def test_change_to_reference_grad(self):
        ...
        actual = change_to_reference_grad(expr)
        expected = as_tensor(Jinv[s, k]*(Jinv[r, j]*(Jinv[q, i]*ReferenceGrad(ReferenceGrad(ReferenceGrad(u)))[q, r, s])), (i, j, k))
        self.assertEqual(renumber_indices(actual), renumber_indices(expected))

        # Multiple grads on a vector valued function
        expr = grad(grad(v))
        actual = change_to_reference_grad(expr)
        expected = as_tensor(Jinv[s, j]*(Jinv[r, i]*ReferenceGrad(ReferenceGrad(v))[t, r, s]), (t, i, j))
        self.assertEqual(renumber_indices(actual), renumber_indices(expected))

        expr = grad(grad(grad(v)))
        actual = change_to_reference_grad(expr)
        expected = as_tensor(Jinv[s, k]*(Jinv[r, j]*(Jinv[q, i]*ReferenceGrad(ReferenceGrad(ReferenceGrad(v)))[t, q, r, s])), (t, i, j, k))
        self.assertEqual(renumber_indices(actual), renumber_indices(expected))
